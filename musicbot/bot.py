    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.voice_members and author_perms.skip_when_absent:
                newmsg = 'Sarim peste dedicatia `%s`: `%s` din partea lui `%s` ca e cygan si nu mai e pe chat' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
                player.skip()
            elif self.config.now_playing_mentions:
                newmsg = '%s ne dedica o melodie de ascultare : `%s`. Cu stima si respect.' % (
                    entry.meta['author'].mention, entry.title)
            else:
                newmsg = 'Din partea mea, %s dedic cea mai tare melodie : `%s` pentru toata lumea prezenta in `%s`!' % (
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            if not player.autoplaylist:
                if not self.autoplaylist:
                    ...
                    log.warning("Nu stiu d-astea boss, ma potolesc.")
                    self.config.auto_playlist = False
                else:
                    log.debug("Nu mai sunt dedicatii. Bag ce vrea muschii mei...")
            ...
            while player.autoplaylist:
                ...
                try:
                    ...
                    if 'YouTube said:' in e.args[0]:
                        ...
                        log.error("Nu stiu sa procesez fa dedicatia iutub:\n{}".format(e.args[0]))

                    else:
                        # Probably an error from a different extractor, but I've only seen youtube's
                        log.error("Nu stiu sa procesez fa dedicatia \"{url}\": {ex}".format(url=song_url, ex=e))
                ...
                if info.get('entries', None):  # or .get('_type', '') == 'playlist'
                    log.debug("Sarim peste pleilistu asta de roacheri.")
            ...
            if not self.autoplaylist:
                # TODO: When I add playlist expansion, make sure that's not happening during this check
                log.warning("Nu am ce melodie pentru ascultare sa pun, ma potolesc.")
                self.config.auto_playlist = False
        ...
        try:
            ...
            if message.author.id != self.config.owner_id:
                if user_permissions.command_whitelist and command not in user_permissions.command_whitelist:
                    raise exceptions.PermissionsError(
                        "Nu faci parte din clanu de barosani ({}).".format(user_permissions.name),
                        expire_in=20)

                elif user_permissions.command_blacklist and command in user_permissions.command_blacklist:
                    raise exceptions.PermissionsError(
                        "Esti in clanu de fraeresteni ({}).".format(user_permissions.name),
                        expire_in=20)
